...
def get_caption(img, processor, lang='en', retries=3, delay=1.0, model_name="google/gemma-3-4b-it"):
    prompt = PROMPT[lang]
    system_msg = helper[lang]

    # Convert to base64 string depending on input type
    if isinstance(img, torch.Tensor):
        img_b64 = tensor_to_base64_image(img, processor)
    else:
        img_b64 = pil_to_base64_image(img)
    ...
    for attempt in range(retries):
        try:
            completion = client.chat.completions.create(
                model=model_name,
                messages=messages,
                extra_headers={"X-Title": "VLM_Attack_Research"}
            )
            return completion.choices[0].message.content.strip()
        except Exception as e:
            print(f"[OpenRouter Error] Retry {attempt + 1}/{retries} — {e}")
            time.sleep(delay)
...
def tensor_to_base64_image(tensor, processor):
    device = tensor.device
    mean, std = get_processor_stats(processor, device)
    unnorm = unnormalize_pixel_values(tensor, mean, std) * 255
    unnorm = torch.clamp(unnorm, 0, 255).byte()
    pil_img = to_pil_image(unnorm.squeeze(0).cpu())
...
def pil_to_base64_image(pil_img):
    buffer = BytesIO()
    pil_img.save(buffer, format="JPEG")
    buffer.seek(0)
    img_bytes = buffer.read()
...
def show_caption_diff(model, processor, image, adv_image, lang='en'):
    text_clean = get_caption(image, processor, lang)
    text_adv = get_caption(adv_image, processor, lang)
...
def process_sample(sample):
    try:
        ...
        print(f'[Thread] Processing sample {sample_id}')
        ...
        print(f"[Error] Sample ID {sample.get('sample_id')}: {e}")
        return None
...
print("Starting multithreaded attack processing...")

with ThreadPoolExecutor(max_workers=max_workers) as executor:
    futures = [executor.submit(process_sample, sample) for sample in dataset]

    for future in tqdm(as_completed(futures), total=len(dataset), desc="Processing Samples"):
